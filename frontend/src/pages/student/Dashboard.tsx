    ...
    <div className="py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {[
            { icon: BookOpen, label: "Active Applications", value: "5", color: "primary", trend: "+2 this week" },
            { icon: Clock, label: "Upcoming Deadlines", value: "3", color: "warning", trend: "Next: 3 days" },
            { icon: Star, label: "Saved Scholarships", value: "12", color: "accent", trend: "4 new matches" },
            { icon: CheckCircle, label: "Completed", value: "8", color: "success", trend: "75% success rate" }
          ].map((stat, i) => (
            <div key={i} className="bg-base-100 rounded-xl shadow-sm hover:shadow-md transition-shadow duration-300 overflow-hidden">
              <div className="p-5">
                <div className="flex items-center">
                  <div className={`flex-shrink-0 p-3 rounded-lg bg-${stat.color}/10`}>
              ...
              <div className="p-6">
                ...
                <div className="space-y-6">
                  {scholarships.map((scholarship, i) => (
                    <div key={i} className="bg-base-100 rounded-lg p-4">
                        ...
                        <div className="ml-4">
                          <div className="flex items-center">
                            <div className="w-48 bg-base-200 rounded-full h-2">
                              <div
                                className={`h-2 rounded-full ${
                                  scholarship.status === "In Progress" ? "bg-primary" :
                                  scholarship.status === "Not Started" ? "bg-secondary" :
                                  "bg-success"
                                }`}
                            ...
                            <span className={`ml-3 text-sm font-medium ${
                              scholarship.status === "In Progress" ? "text-primary" :
                              scholarship.status === "Not Started" ? "text-secondary" :
                              "text-success"
                            }`}>
                              {scholarship.status}
                            </span>
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <div key={i} className="flex items-start space-x-3 p-3 hover:bg-gray-50 rounded-lg transition-colors duration-150">
                      <div className={`flex-shrink-0 p-2 rounded-lg bg-${notification.color}/10`}>
                        <notification.icon className={`h-5 w-5 text-${notification.color}`} />
                      </div>
